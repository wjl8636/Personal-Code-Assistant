...
from personalcode.teams.progress import TeammateProgress, random_verb

if TYPE_CHECKING:
    from personalcode.agent import Agent
    from personalcode.conversation import ConversationManager
    from personalcode.teams.models import TeammateInfo
        ...
        for m in mailbox.consume(member_name):
            ...
            if (m.type == protocol.PLAN_APPROVAL_RESPONSE
                ...
                if protocol.approved(m):
                    # 批准后切回正常权限，队友可以改文件了
                    from personalcode.permissions.modes import PermissionMode
    ...
    async def _run() -> str:
        ...
        try:
            if conversation is not None:
                ...
                from personalcode.conversation import ConversationManager as CM
                conv = CM()
