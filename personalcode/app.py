...
from personalcode.client import (
    AuthenticationError,
    LLMClient,
    LLMError,
    create_client,
...
from personalcode.commands import (
    CommandContext,
    CommandRegistry,
    complete,
    parse_command,
...
from personalcode.commands.completion import CompletionPopup
from personalcode.commands.handlers import register_all_commands
from personalcode.config import MCPServerConfig, ProviderConfig
from personalcode.hooks import HookContext, HookEngine, load_hooks
from personalcode.conversation import ConversationManager, Message
from personalcode.mcp import ConnectResult, MCPManager
from personalcode.mcp.tool_wrapper import mcp_tool_name_prefix
from personalcode.memory import (
    MemoryManager,
    Session,
    SessionManager,
    RecallResult,
...
from personalcode.agents.task_manager import TaskManager
from personalcode.agents.trace import TraceManager
from personalcode.agents.notification import inject_task_notifications
from personalcode.commands.handlers.tasks import create_tasks_command
from personalcode.skills.executor import SkillExecutor
from personalcode.skills.loader import SkillLoader
from personalcode.commands.handlers.skill_register import register_skill_commands
from rich.text import Text as RichText
from textual.theme import Theme
from personalcode.tools import ToolRegistry, create_default_registry
from personalcode.tools.agent_tool import AgentTool
from personalcode.tools.ask_user import AskUserEvent, AskUserTool
from personalcode.tools.impl.tool_search import ToolSearchTool
from personalcode.tools.mcp_call import McpCallTool
from personalcode.tools.install_skill import InstallSkillTool
from personalcode.tools.load_skill import LoadSkill
from personalcode.worktree.cleanup import start_stale_cleanup_task
from personalcode.worktree.manager import WorktreeManager
from personalcode.commands.handlers.worktree import create_worktree_command
from personalcode.teammate_tree import TeammateTree
...
_SKIP_DIRS = {".git", "node_modules", ".venv", "__pycache__", ".personalcode", "build", ".gradle"}
...
class ChatInput(TextArea):
    ...
    def load_history(self, work_dir: str) -> None:
        self._history_file = Path(work_dir) / ".personalcode" / "history"
...
class PersonalCodeApp(App):
    ...
    def __init__(
        ...
        from personalcode.config import SandboxAppConfig
    ...
    def on_mount(self) -> None:
        self.register_theme(_PERSONALCODE_THEME)
        self.theme = "personalcode"
        if len(self.providers) == 1:
    ...
    def _select_provider(self, provider: ProviderConfig) -> None:
        ...
        checker = PermissionChecker(
            ...
            rule_engine=RuleEngine(
                user_rules_path=home / ".personalcode" / "permissions.yaml",
                project_rules_path=Path(work_dir) / ".personalcode" / "permissions.yaml",
                local_rules_path=Path(work_dir) / ".personalcode" / "permissions.local.yaml",
            ),
        ...
        if self._sandbox_cfg.enabled:
            from personalcode.sandbox import SandboxConfig, create_sandbox
            os_sandbox = create_sandbox()
            if os_sandbox and os_sandbox.available():
                sandbox_config = SandboxConfig(
                    allow_write=[work_dir, "/tmp"],
                    deny_write=[
                        f"{work_dir}/.personalcode/config.yaml",
                        f"{work_dir}/.personalcode/permissions.local.yaml",
                    ],
        ...
        from personalcode.filehistory import FileHistory
        ...
        from personalcode.tools.exit_plan_mode import ExitPlanModeTool
        ...
        # --- Worktree 系统初始化 ---
        from personalcode.config import WorktreeConfig
        ...
        from personalcode.tools.enter_worktree import EnterWorktreeTool
        from personalcode.tools.exit_worktree import ExitWorktreeTool
        ...
        from personalcode.tools.team_create import TeamCreateTool
        from personalcode.tools.team_delete import TeamDeleteTool
        ...
        from personalcode.commands.handlers.trace import create_trace_command
        ...
        from personalcode.tools.send_message import SendMessageTool
        from personalcode.tools.synthetic_output import SyntheticOutputTool
        from personalcode.tools.task_stop import TaskStopTool
    ...
    def _flush_session(self) -> None:
        ...
        按消息上的标记跳过已写的，不按下标切片。环境上下文和长期记忆是 insert(0)
        插到历史最前面的，一插索引就整体后移，记下的下标会跟着错位，把已经写过的
        消息再写一遍。首轮两处插入正好错开两位，用户那句话和第一条提醒都会重复。
        """
        if not self.session:
            return
    ...
    async def _prefetch_relevant_memories(self, query: str) -> RecallResult:
        ...
        rendered system-reminder body plus the selected memory paths, or an
        empty result on any failure / timeout.
        """
        ...
        async def selector(system_prompt: str, user_message: str) -> str:
            from personalcode.tools.base import StreamEnd, TextDelta
        ...
        try:
            ...
            return RecallResult.of(results)
    ...
    async def _send_message(self, text: str, is_notification: bool = False) -> None:
        ...
        if text:
            ...
            if self.session:
                # 立刻落盘，别等这一轮跑完：中途崩了用户这句话也不能丢。
                # 标记成已写，后面刷盘时就会跳过它，不会写第二遍。
                self.conversation.history[-1].persisted = True
        ...
        try:
            async for event in self.agent.run(self.conversation):
                if isinstance(event, ThinkingText):
                    ...
                    # 压缩已重写整个历史，摘要之前的内容都在 boundary 记录里了，
                    # 这里把重建后的消息全部标成已写，刷盘时只追加 boundary 之后的新消息
                    for msg in self.conversation.history:
                        msg.persisted = True
    ...
    async def _show_plan_approval(self) -> None:
        from personalcode.plan_dialog import InlinePlanWidget
    ...
    def on_inline_plan_widget_responded(
        ...
        from personalcode.plan_dialog import InlinePlanWidget, PlanChoice
        from personalcode.prompts import build_plan_mode_exit_reminder
    ...
    async def _handle_askuser(self, event: AskUserEvent) -> None:
        from personalcode.askuser_dialog import InlineAskUserWidget
    ...
    def on_inline_permission_widget_responded(
        ...
        if req is not None and not req.future.done():
            req.future.set_result(event.reply)
        self._dismiss_permission_widget()

    def _dismiss_permission_widget(self) -> None:
        """移除确认框并恢复输入框。答复后和本轮被中断时都会调用。"""
        from personalcode.permission_dialog import InlinePermissionWidget

        self._pending_perm_request = None
        # 从聊天区移除权限弹窗组件
        for widget in self.query("#perm-inline").results(InlinePermissionWidget):
            widget.remove()
        # 重新启用输入框
        try:
            self.query_one("#chat-input").disabled = False
    ...
    async def _init_mcp(self) -> None:
        ...
        if self._selected_provider is not None:
            from personalcode.mcp.loading_strategy import decide_and_apply
        ...
        if server_count > 0 and mcp_tools > 0:
            # 构建 MCP 指令，用各服务器连接时返回的 instructions
            parts = []
    ...
    async def action_handle_ctrl_c(self) -> None:
        if self._streaming:
            if self._agent_task and not self._agent_task.done():
                self._agent_task.cancel()
            # 确认框还开着就一并移除，本轮取消后 Agent 不会再等它的答复
            self._dismiss_permission_widget()
            self._show_system_message("(response interrupted)")
            self._finish_streaming()
            try:
