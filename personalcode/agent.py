...
from personalcode.context import (
    CompactBoundary,
...
from personalcode.conversation import ConversationManager, ToolResultBlock, ToolUseBlock
from personalcode.conversation_pairing import rejected_tool_result
from personalcode.conversation import ThinkingBlock as ConvThinkingBlock
...
from personalcode.hooks import HookContext, HookEngine, ToolRejectedError
from personalcode.hooks.engine import HookNotification
...
class Agent:
    ...
    def _announce_deferred_tools(self, conversation: ConversationManager) -> None:
        ...
        from personalcode.mcp.loading_strategy import McpLoadingMode
    ...
    def _transcript_path(self) -> str:
        if self.session_id:
            return str(Path(self.work_dir) / ".personalcode" / "sessions" / f"{self.session_id}.jsonl")
        return ""
    ...
    def _get_plan_path(self) -> Path:
        ...
        plans_dir = Path(self.work_dir) / ".personalcode" / "plans"
        ...
        if self.permission_checker:
            ...
            if decision.effect == "ask":
                ...
                if reply.response == PermissionResponse.ALLOW_ALWAYS:
                    from personalcode.permissions.rules import Rule, extract_content
    ...
    def _maybe_persist_or_truncate(
        ...
        from personalcode.context.manager import (
