...
SELECTOR_SYSTEM_PROMPT = (
    "You are selecting memories that will be useful to PersonalCode as it processes "
    ...
    "PersonalCode as it processes the user's query (up to 5). Only include memories "
    ...
    "that are usage reference or API documentation for those tools (PersonalCode is "
...
class RecallResult:
    ...
    @classmethod
    def of(cls, memories: list[RelevantMemory]) -> "RecallResult":
        """把选中的记忆渲染成 reminder，并带上它们的路径。"""
        if not memories:
            return cls()
        return cls(reminder=render_reminder(memories), paths=[m.path for m in memories])
