...
"""从 .personalcode/commands/ 目录加载自定义 Markdown 命令。
...
from personalcode.commands.registry import Command, CommandContext, CommandType
...
def load_user_commands(work_dir: str) -> list[Command]:
    ...
    搜索路径（后者覆盖前者）：
      1. ~/.personalcode/commands/
      2. <work_dir>/.personalcode/commands/
    """
    dirs: list[str] = []
    home = Path.home()
    dirs.append(str(home / ".personalcode" / "commands"))
    dirs.append(str(Path(work_dir) / ".personalcode" / "commands"))
