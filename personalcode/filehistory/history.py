...
class FileHistory:
    ...
    def __init__(self, base_dir: str, session_id: str) -> None:
        self._session_dir = Path(base_dir) / ".personalcode" / "file-history" / session_id
    ...
    def rewind(self, snapshot_index: int) -> list[str]:
        with self._lock:
            ...
            # 就该删掉，不能留在磁盘上。
            created_after_target = set(self._tracked) - set(target.backups)
            for file_path in created_after_target:
                fp = Path(file_path)
                if fp.exists():
                    fp.unlink()
                    changed.append(file_path)
