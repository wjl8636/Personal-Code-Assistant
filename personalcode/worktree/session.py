...
def _session_path(personalcode_dir: Path) -> Path:
    return personalcode_dir / SESSION_FILENAME


def save_worktree_session(
    personalcode_dir: Path,
    session: WorktreeSession | None,
) -> None:
    path = _session_path(personalcode_dir)
    if session is None:
...
def load_worktree_session(personalcode_dir: Path) -> WorktreeSession | None:
    path = _session_path(personalcode_dir)
    if not path.exists():
        return None
    try:
