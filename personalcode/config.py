...
def load_config(path: Path | None = None) -> AppConfig:
    ...
    home = Path.home()
    candidates = [
        home / ".personalcode" / "config.yaml",
        cwd / ".personalcode" / "config.yaml",
        cwd / ".personalcode" / "config.local.yaml",
    ]
    ...
    if merged is None:
        raise ConfigError(
            "No config file found. Expected .personalcode/config.yaml "
            "in project or ~/.personalcode/config.yaml"
        )
    return merged
