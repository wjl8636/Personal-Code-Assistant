...
from personalcode.config import ConfigError, load_config
from personalcode.hooks import HookConfigError, HookEngine, load_hooks
from personalcode.permissions import PermissionMode


def main() -> None:
    # 队友 worker 模式：由 tmux/iTerm2 窗格用 `-m personalcode --teammate ...` 拉起。
    ...
    # 先确保 .personalcode/ 目录存在，否则下面写 debug.log 会因目录不存在而崩溃
    Path(".personalcode").mkdir(parents=True, exist_ok=True)
    # 追加写：排查异常退出要看的往往是上一次运行的日志，覆盖会把现场冲掉
    logging.basicConfig(
        level=logging.INFO,
        format="%(asctime)s %(name)s %(message)s",
        filename=".personalcode/debug.log",
        filemode="a",
    )
    crashlog.install()

    parser = argparse.ArgumentParser(prog="personalcode", description="PersonalCode AI coding assistant")
    parser.add_argument(
        "--mode",
    ...
    if args.remote:
        from personalcode.remote import RemoteServer
    ...
    from personalcode.driver import NoAltScreenDriver
...
async def _run_prompt(config, permission_mode, hook_engine, prompt: str, output_format: str = "text") -> None:
    ...
    from personalcode.client import create_client, resolve_context_window
    from personalcode.conversation import ConversationManager
    from personalcode.memory.instructions import load_instructions
    ...
    from personalcode.worktree import WorktreeManager
    ...
    if config.mcp_servers:
        from personalcode.mcp import MCPManager
...
async def _build_teammate_registry(
    ...
    from personalcode.tools.send_message import SendMessageTool
    from personalcode.tools.synthetic_output import SyntheticOutputTool
    from personalcode.tools.task_create import TaskCreateTool
    from personalcode.tools.task_get import TaskGetTool
    from personalcode.tools.task_list import TaskListTool
    from personalcode.tools.task_update import TaskUpdateTool
    from personalcode.worktree import WorktreeManager
...
async def _run_teammate(team_name: str, agent_name: str) -> None:
    ...
    from personalcode.teams.spawn_inprocess import LEAD_NAME, spawn_inprocess_teammate
    ...
    checker = PermissionChecker(
        ...
        sandbox=PathSandbox(work_dir),
        rule_engine=RuleEngine(
            user_rules_path=Path.home() / ".personalcode" / "permissions.yaml",
            project_rules_path=Path(work_dir) / ".personalcode" / "permissions.yaml",
            local_rules_path=Path(work_dir) / ".personalcode" / "permissions.local.yaml",
        ),
